...
router.post('/respond', oapi.path({
  ...
  const marvResponse = await openai.createCompletion({
    model: "text-davinci-002",
    prompt: "Marlon is a chatbot that reluctantly answers questions with sarcastic responses:\n\n" + req.body.messages,
    temperature: 0.5,
    max_tokens: 60,
    top_p: 0.3,
  ...
  const response = req.body.messages + "\nMarlon: " + marvResponse;
